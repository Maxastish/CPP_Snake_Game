...
void showField(Field &f){
	...
	else if (choice_color == 2){
		#define COLOR_CYAN 3
		console.TextColor(COLOR_CYAN);
	}
	else if (choice_color == 3){
		#define COLOR_LIGHTMAGENTA 13
		console.TextColor(COLOR_LIGHTMAGENTA);
	}
...
void addMenuItem(Menu & m, string text, MenuAction ma){
	...
	mi.text = text;
	mi.action = ma;
	m.items.push_back(mi); //вставка пункта в конец
...
void prevMenuItem(Menu &m ){
	m.activeItem--;
	if (m.activeItem == -1){
		m.activeItem = m.count-1;
	}
};

void nextMenuItem(Menu &m){
	m.activeItem++;
	if (m.activeItem == m.count){
		m.activeItem = 0;
	}
};

/*
void initColourMenu(Menu & m, string caption){
	m.caption = caption;
	m.activeItem = -1;
};

void addColourMenuItem(Menu & m, string text, MenuAction ma){
	MenuItem mi;
	mi.text = text;
	mi.action = ma;
	m.items.push_back(mi); //вставка пункта в конец
...
void prevColourMenuItem(Menu &m ){
	m.activeItem--;
	if (m.activeItem == -1){
		m.activeItem = m.count-1;
	}
...
void nextColourMenuItem(Menu &m){
	m.activeItem++;
	if (m.activeItem == m.count){
		m.activeItem = 0;
	}
...
void settings(){
	...
	cout << "Settings";
	/*Menu menu;
	initMenu(menu, "Colours");
	addMenuItem(menu, "1", playSnake);
	addMenuItem(menu, "2", records);*/

	cout << "Change the wall's color" << endl;
	cout << endl;
	cout << "1) YELLOW" << endl;
	cout << "2) CYAN" << endl;
	cout << "3) LIGHTMAGENTA" << endl;
	cout << endl;
	cout << "1,2,3 ?" << endl;
	cout << endl;
	cin >> choice_color;
...
void showMenuItem(string text, bool isActive){
	console.TextColor(COLOR_WHITE);
	if (isActive) {
		console.TextBackground(COLOR_GREEN);
	}
	else{
		console.TextBackground(COLOR_BLACK);
	}
...
void showMenu(Menu &m){
	console.TextBackground(COLOR_WHITE);
	console.TextColor(COLOR_BLACK);
	console.GotoXY(36, 6);
	cout << m.caption;
	for (int i = 0; i < m.count; i++){
		console.GotoXY(36, 2*i + 9);
		showMenuItem(m.items[i].text, i == m.activeItem);
	}
}
...
runMenu(Menu &m){
	...
	while (isRunning){
		Key key = getKey();
		switch(key){
			case kUp: prevMenuItem(m);
			break;

			case kDown: nextMenuItem(m);
			break;

			case kEnter:
				if (m.items[m.activeItem].action != 0) {
...
int main(){
	Menu menu;
	initMenu(menu, "Menu");
	addMenuItem(menu, "Play", playSnake);
	addMenuItem(menu, "Records", records);
	addMenuItem(menu, "Achivements", achivements);
	addMenuItem(menu, "Settings", settings);
	addMenuItem(menu, "Exit", 0);
	runMenu(menu);
	return 0;
}
